  ...
  const reverseGeocode = async (latitude: number, longitude: number) => {
    try {
      const response = await axios.get(
        `https://api.geocod.io/v1.7/reverse?q=${latitude},${longitude}&api_key=680ffee060f6ee79ee77e9888068696fee888f8`,
      );
      ...
      if (response.data.results && response.data.results.length > 0) {
        ...
        const address = result.formatted_address; // Full address
        const city = addressComponents.city || '';
        const sublocality = addressComponents.sublocality || '';
        const postalCode = addressComponents.zip || '';
    ...
    return new Promise(async resolve => {
      try {
        ...
        if (!hasPermission) {
          ...
          resolve({locationName: 'Manado', latitude: '', longitude: ''});
          return;
        }
